...
export function SocialFeedPage() {
  ...
  const [selectedMedia, setSelectedMedia] = useState<SocialPostMedia | null>(null)
  ...
  const feedPostsRaw = feedData?.pages.flatMap((page) => page.posts) ?? []
  const seenPostIds = new Set<string>()
  const feedPosts = feedPostsRaw.filter((post) => {
  ...
  const postsResolved = feedPosts.map((post) => {
  ...
  const postsResolvedWithShared = postsResolved.map((post) => {
    const shared = post.sharedPost
  ...
  const posts = activeTab === 'following' ? postsResolvedWithShared : postsResolvedWithShared
  ...
  const handleMediaPress = (post: SocialPost, index: number, mediaList?: SocialPostMedia[]) => {
    const media = mediaList?.[index] ?? post.media?.[index]
    if (!media) return
  ...
  return (
    <View className='flex-1 bg-gray-50'>
      ...
      <FlatList
        ...
        ListFooterComponent={
          isFetchingNextPage ? (
            <Text className='text-center text-gray-500 py-3'>Đang tải thêm bài viết...</Text>
          ) : null
        }
        ListEmptyComponent={
                ...
                <View className='px-4 py-3'>
                  <View className='flex-row items-start'>
                    ...
                    <View className='ml-3 flex-1'>
                      <View className='self-start px-0 py-0'>
                        <Text className='font-semibold text-gray-900 text-[16px]'>
                          {displayName}
                        </Text>
                        <Text className='text-gray-700 mt-1 text-[16px]'>{item.content}</Text>
                      </View>
                      ...
                      <View className='flex-row items-center mt-2 ml-1'>
                        ...
                        <TouchableOpacity className='mr-4' onPress={() => handleCommentLike(item)}>
                          <Text
                            className={`font-semibold ${item.currentUserReaction === 'LIKE' ? 'text-blue-600' : 'text-gray-600'}`}
                          >
                            Like
                          </Text>
                        </TouchableOpacity>
                        <TouchableOpacity className='mr-4' onPress={() => handleCommentReply(item, displayName)}>
                          <Text className='text-gray-600 font-semibold'>Reply</Text>
                        </TouchableOpacity>
                        {isOwnComment && (
                          <TouchableOpacity onPress={() => handleDeleteComment(item.id)}>
                            <Text className='text-gray-600 font-semibold'>Delete</Text>
                          </TouchableOpacity>
                        )}
                      </View>
                    </View>
