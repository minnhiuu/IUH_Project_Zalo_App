...
export function MyProfilePage({
  ...
  const { data, hasNextPage, fetchNextPage, isFetchingNextPage, isLoading, isError, refetch } = useInfiniteMyPosts(20)
          ...
          <View className='bg-white dark:bg-zinc-900'>
            ...
            <View className='px-4 pb-4'>
              ...
              <View className='flex-row items-end gap-3 -mt-12 mb-4'>
                ...
                <TouchableOpacity onPress={onEditProfile} className='flex-1 bg-blue-500 rounded-lg py-2'>
                  <Text className='text-white font-semibold text-center'>Edit Profile</Text>
                </TouchableOpacity>
              </View>

              {/* Name & Bio */}
              <Text className='text-xl font-bold text-zinc-900 dark:text-white mb-1'>{userName}</Text>
              <Text className='text-sm text-zinc-600 dark:text-zinc-400 mb-4'>{bio}</Text>

              {/* Stats */}
              <View className='flex-row justify-around py-3 border-t border-b border-zinc-200 dark:border-zinc-700'>
                <View className='items-center'>
                  <Text className='text-lg font-bold text-zinc-900 dark:text-white'>{posts.length}</Text>
                  <Text className='text-xs text-zinc-500'>Posts</Text>
                </View>
                <View className='items-center'>
                  <Text className='text-lg font-bold text-zinc-900 dark:text-white'>0</Text>
