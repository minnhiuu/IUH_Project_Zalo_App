...
export function SuggestedFriendsSidebar({ friends = [], onAddFriend }: SuggestedFriendsSidebarProps) {
  return (
    <ScrollView className='bg-white dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-700 w-80'>
      <View className='p-4'>
        <Text className='text-base font-bold text-zinc-900 dark:text-white mb-4'>People You May Know</Text>

        <View className='gap-3'>
          {friends.map((friend) => (
            <View key={friend.id} className='border border-zinc-200 dark:border-zinc-700 rounded-lg p-3'>
              <View className='flex-row items-center gap-3 mb-2'>
                {friend.avatar ? (
                  ...
                  <View className='w-10 h-10 rounded-full bg-blue-400 items-center justify-center'>
                    <Text className='text-white font-bold'>{friend.name.charAt(0).toUpperCase()}</Text>
                  </View>
                )}
                <View className='flex-1'>
                  <Text className='text-sm font-semibold text-zinc-900 dark:text-white'>{friend.name}</Text>
                  <Text className='text-xs text-zinc-500'>{friend.mutualFriends} mutual friends</Text>
                </View>
              </View>
              <TouchableOpacity
