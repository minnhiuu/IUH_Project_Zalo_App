...
export function ReactionPeopleModal({ visible, postId, onClose }: ReactionPeopleModalProps) {
  const [selectedTab, setSelectedTab] = useState('all')

  const filteredReactions =
    selectedTab === 'all' ? MOCK_REACTIONS : MOCK_REACTIONS.filter((r) => r.reaction === selectedTab)
          ...
          {REACTION_TABS.map((tab) => (
            <TouchableOpacity
              ...
              className={`px-4 py-3 border-b-2 ${
                selectedTab === tab.id ? 'border-b-blue-500' : 'border-b-transparent'
              }`}
            >
              <Text className={`text-sm font-medium ${selectedTab === tab.id ? 'text-blue-500' : 'text-zinc-500'}`}>
