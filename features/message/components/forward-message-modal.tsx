...
export function ForwardMessageModal({
  ...
  const sections = useMemo(
    () => [
      { title: t('message.forward.recentChats', { defaultValue: 'Trò chuyện gần đây' }), data: directConversations.slice(0, 8) },
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center' }}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ fontSize: 16 * 0.95, color: colors.textSecondary, marginTop: 1 }}>
                  {t('message.forward.selectedCount', { defaultValue: 'Đã chọn: {{count}}', count: selectedIds.length })}
                </Text>
              </View>
            </View>
              ...
              <View>
                <View style={{ paddingHorizontal: 16, paddingTop: 6, paddingBottom: 10 }}>
                  ...
                  <View style={{ flexDirection: 'row', marginTop: 16, marginBottom: 10 }}>
                    {[
                      { key: 'newGroup', icon: 'people-outline', label: t('message.forward.newGroup', { defaultValue: 'Nhóm mới' }) },
                      { key: 'journal', icon: 'time-outline', label: t('message.forward.journal', { defaultValue: 'Nhật ký' }) },
                      { key: 'otherApps', icon: 'share-social-outline', label: t('message.forward.otherApps', { defaultValue: 'App khác' }) },
                    ].map((item) => (
                      <View key={item.key} style={{ width: 110, alignItems: 'center' }}>
                        ...
                        <Text style={{ marginTop: 9, fontSize: 13, color: isDark ? '#C7CDD8' : colors.textSecondary }}>
                          {item.label}
                        </Text>
                      </View>
                    ))}
            ...
            ListFooterComponent={
              directConversations.length > 8 ? (
                <TouchableOpacity activeOpacity={0.8} style={{ alignItems: 'center', paddingVertical: 14 }}>
                  <Text style={{ color: isDark ? '#D1D5DB' : '#374151', fontSize: 15, fontWeight: '600', letterSpacing: 0.3 }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', minHeight: 48 }}>
              ...
              >
                <Ionicons
                  name='send-sharp'
                  size={19}
                  color={selectedIds.length ? '#fff' : colors.textSecondary}
                />
              </TouchableOpacity>
            </View>
          </View>
