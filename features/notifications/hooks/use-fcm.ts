...
async function registerNotificationCategories(Notifications: NotificationsModule) {
...
async function setupNotificationHandler(Notifications: NotificationsModule) {
  if (notificationHandlerRegistered) return
  notificationHandlerRegistered = true

  const { displayChatNotification } = await import('@/tasks/notifee-background-handler')
...
export const useFcm = () => {
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      await setupNotificationHandler(Notifications)
    ...
    setup().catch((e) => console.error('[FCM] Error setting up notification categories:', e))
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      const [Notifications, notifeeModule] = await Promise.all([getNotificationsModule(), getNotifeeModule()])
      if (!Notifications || !notifeeModule || !isMounted) return
      ...
      responseListener.current = Notifications.addNotificationResponseReceivedListener((response) => {
        ...
        // Dismiss the notification after it is opened.
    ...
    setup().catch((error) => console.error('[FCM] Error setting up native notification listeners:', error))
