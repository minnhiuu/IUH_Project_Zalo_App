...
const MENU_ITEMS_KEYS: MenuItem[] = [
  ...
  {
    id: '4',
    titleKey: 'profile.menu.storage',
    subtitleKey: 'profile.menu.storageDesc',
    icon: 'pie-chart-outline',
    route: '/settings/data-on-device'
  },
  { id: '5', titleKey: 'profile.menu.qrWallet', subtitleKey: 'profile.menu.qrWalletDesc', icon: 'wallet-outline' },
  {
    id: '6',
    titleKey: 'profile.menu.accountSecurity',
        ...
        <Card style={{ marginTop: 8 }}>
          {MENU_ITEMS_KEYS.map((item, index) => (
            <VStack key={item.id}>
              ...
              {/* Divider */}
              {index < MENU_ITEMS_KEYS.length - 1 && <Divider style={{ marginLeft: 64 }} />}
            </VStack>
          ))}
        </Card>
